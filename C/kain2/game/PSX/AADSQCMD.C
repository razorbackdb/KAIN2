...

{
  ...
  if ((bVar1 & 7) == 0) {
    aadUpdateSlotVolPan((_AadSequenceSlot *)event);
    return;
  }
  if ((uint)event->statusByte - 0x41 < 3) {
    aadUpdateSlotVolPan((_AadSequenceSlot *)event);
    return;
  }
  bVar3 = bVar1;
...

{
  ...
  if ((byte)event->dataByte[1] < 0x80) {
    iVar1 = loadFromHead + (uint)(byte)event->dataByte[1];
    (&DAT_00001c08)[iVar1] = (&DAT_00001c08)[iVar1] & ~event->dataByte[0];
  }
  return;
}
  ...
  if (*(char *)(iVar2 + 0x338) != '\0') {
    do {
      ...
      *(char *)(iVar2 + 0x338) = *(char *)(iVar2 + 0x338) + -1;
      ...
      iVar3 = (int)&(slot->tempo).currentTick + track;
    } while (*(char *)(iVar2 + 0x338) != '\0');
    *(byte *)(iVar3 + 0x3d8) = *(byte *)(iVar3 + 0x3d8) & 0xe7;
    return;
  }
  metaCmdUpdateMute((AadSeqEvent *)slot,(_AadSequenceSlot *)track);
  return;
}
...

{
  if ((byte)event->dataByte[1] <=
      (byte)(&DAT_00001c08)[DAT_800d07bc + (uint)(byte)event->dataByte[0]]) {
    metaCmdGetChannelBendRange((AadSeqEvent *)slot,slot);
    return;
  }
                    /* WARNING: Subroutine does not return */
  aadGotoSequenceLabel(slot,(uint)event->track,(uint)(byte)event->dataByte[2]);
...

{
  if ((byte)(&DAT_00001c08)[(int)&aadMem->updateCounter + (uint)(byte)event->dataByte[0]] <=
    ...
    metaCmdGetSequenceAssigned((AadSeqEvent *)slot,slot);
    return;
  }
                    /* WARNING: Subroutine does not return */
  aadGotoSequenceLabel(slot,(uint)event->track,(uint)(byte)event->dataByte[2]);
...

{
  if (((&DAT_00001c08)[(int)&aadMem->updateCounter + (uint)(byte)event->dataByte[0]] &
      event->dataByte[1]) == 0) {
    metaCmdSetVariableBits((AadSeqEvent *)slot,slot);
    return;
  }
                    /* WARNING: Subroutine does not return */
  aadGotoSequenceLabel(slot,(uint)event->track,(uint)(byte)event->dataByte[2]);
