...

{
  ...
  if (p_Var6 != (_G2AnimInterpInfo_Type *)0x0)
  {
    ...
    {
      lVar4 = _G2AnimAlphaTable_GetValue(p_Var6->alphaTable, ((int)(uVar7 << 0x10) >> 4) / (int)p_Var6->duration);
...
void G2AnimSection_SwitchToKeylistAtTime(_G2AnimSection_Type *section, _G2AnimKeylist_Type *keylist, int keylistID,
                                         short targetTime)

{
  ...
  if (section->firstSeg == '\0')
  {
    ...
    {
      G2Anim_GetRootMotionOverInterval(anim, section->storedTime, section->elapsedTime, (_G2SVector3_Type *)&local_28);
    }
  ...
  if ((section->flags & 1) == 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (section->firstSeg == '\0')
        {
          G2Anim_GetRootMotionOverInterval(anim, intervalStart, p_Var4->duration, (_G2SVector3_Type *)&local_20);
  ...
  if (((section->flags & 1) == 0) && ((section->alarmFlags & 1) == 0))
  {
    ...
    while (true)
    {
      ...
      else
      {
        ...
        _G2AnimSection_TriggerEffects(section, (short)(((uint)(ushort)section->loopStartTime - 1) * 0x10000 >> 0x10),
...
void _G2Anim_BuildSegTransformNoControllers(_G2Matrix_Type *segMatrix, _G2Matrix_Type *parentMatrix,
            ...
            else
            {
              if (bVar7 < 0x41)
              {
                if (bVar7 == 0)
                {
                  ...
                  local_58.chanData = local_98.chanData;
                  local_48.chanData = local_98.chanData;
                  local_38[0] = (_G2AnimChanStatus_Type)((uint)local_38[0] & 0xffff0000 | (uint) * (ushort *)pp_Var5);
                }
                else
                {
                  if (bVar7 == 0x20)
                  {
                    local_38[0] = (_G2AnimChanStatus_Type)((uint)local_38[0] & 0xffff0000 |
