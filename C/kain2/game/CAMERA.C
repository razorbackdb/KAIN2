    ...
    case 5:
      ...
      if (pMVar5 != (MultiSpline *)0x0)
      {
        if ((mode == 4) || (mode == 2))
        {
          ...
          local_20[0] = SplineGetNearestPoint(((camera->data).Cinematic.posSpline)->positional, &_Stack40,
                                              &local_38);
...

{
  ...
  iVar1 = COLLIDE_IntersectLineAndPlane_S(&_Stack24, vertex0, vertex1, &camera_plane,
...
void CAMERA_SplineGetNearestPoint2(Camera *camera, Spline *spline, _SVector *point, int *currkey, _SVector *ret_dpoint)
...

{
  ...
  iVar4 = COLLIDE_NearestPointOnLine_S((uint *)&local_60, (uint *)&local_40, (uint *)&local_38, (uint *)point);
  ...
  LAB_8001adb8:
    ...
    else
    {
      ...
      iVar3 = COLLIDE_NearestPointOnLine_S((uint *)&local_50, (uint *)&local_48, (uint *)&local_40, (uint *)point);
      if (0 < iVar4)
        goto LAB_8001ae08;
      *currkey = iVar9;
    }
  ...
  else
  {
    ...
    iVar2 = COLLIDE_NearestPointOnLine_S((uint *)&local_58, (uint *)&local_38, (uint *)&local_30, (uint *)point);
    if (iVar4 < 0x1000)
      goto LAB_8001adb8;
    *currkey = iVar7;
  LAB_8001ae30:
...

{
  ...
  CAMERA_CalcPosition((_Position *)&_Stack40, &camera->targetFocusPoint, &splinecam_helprot,
                      camera->targetFocusDistance);
  CAMERA_SplineGetNearestPoint2(camera, pMVar4->positional, &_Stack40, &(camera->data).Cinematic.splinecam_helpkey,
                                &_Stack48);
  ...
  sVar1 = CAMERA_AngleDifference((p_Var5->rotation).z,
...

{
  ...
  if (pMVar3 != (MultiSpline *)0x0)
  {
    ...
    if (camera->mode == 4)
    {
      CAMERA_SplineGetNearestPoint2(camera, pMVar3->positional, point, &(camera->data).Cinematic.splinecam_currkey,
                                    &local_30);
    }
    else
    {
...

{
  ...
  {
    sVar1 = CAMERA_AngleDifference((camera->focusRotation).z,
                                   (short)(((uint)(ushort)(camera->focusInstance->rotation).z + 0x400) * 0x10000 >> 0x10));
    sVar2 = CAMERA_AngleDifference((camera->focusRotation).z,
                                   (short)(((uint)(ushort)(camera->focusInstance->rotation).z - 0x400) * 0x10000 >> 0x10));
    if (sVar1 < sVar2)
    {
...

{
  ...
  if (roll_inc != 0)
  {
    ...
    sVar1 = CAMERA_AngleDifference((short)((uint)(lVar3 << 4) >> 0x10), (short)((uint)(lVar5 << 4) >> 0x10));
    iVar6 = iVar2;
    if (iVar2 < 0)
    {
      iVar6 = iVar2 + 0xfff;
...

{
  ...
  uVar6 = (uint)(ushort)camera->extraXRot +
              (uint)(ushort)(camera->targetFocusRotation).x + (uint)(ushort)camera->tilt &
          0xfff;
...

{
      ...
      ((camera->instance_mode & 0x3002043) == 0))
  {
    ...
    uVar1 = CAMERA_SignedAngleDifference((short)(((uint)(ushort)(camera->focusInstance->rotation).z + 0x800) * 0x10000 >> 0x10), (camera->core).rotation.z);
    if (iVar3 < 0x17)
    {
