  ...
  if ((AlgoControlFlag & 1U) == 0)
  {
    ...
    else
    {
      while (true)
      {
        ...
        G2EmulationSetInterpController_Vector(instance, segNumber, 8, (_G2SVector3_Type *)&local_18, (segNumber - 0x32) * 3, 2);
        bVar2 = bVar2 + 1;
        if (0x35 < bVar2)
          break;
        pMVar1 = instance->matrix;
...

{
  ...
  G2EulerAngles_FromMatrix(&_Stack24, (instance->anim).segMatrices + ControllerData->segment + -1, 0x15);
  G2EmulationSetInterpController_Vector(instance, ControllerData->segment, ControllerData->type, (_G2SVector3_Type *)&_Stack24, 5, 2);
  return;
}
...

{
  ...
  if (uVar6 != 0)
  {
    ...
    iVar3 = INSTANCE_SetStatsData(Player, (_Instance *)0x0, &Raziel.Senses.lookAtPoint, &eStack40, &MStack72);
