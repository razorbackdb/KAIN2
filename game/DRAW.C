...
/* WARNING: Removing unreachable block (ram,0x8002a530) */
...

{
  ...
  button->textureW = *(short *)(plVar3 + 2);
  button->textureH = *(short *)((int)plVar3 + 10);
  if (local_1c < button->textureW) {
    local_1c = button->textureW;
  }
  iVar2 = (uint)(ushort)button->textureH + (uint)(iVar2 == 8);
  sStack26 = (short)iVar2;
  p_Var1 = VRAM_CheckVramSlot((short *)&uStack32,(short *)(&uStack32 + 1),local_1c,
                              (short)((uint)(iVar2 * 0x10000) >> 0x10),4,0);
  button->vramBlock = p_Var1;
  button->tpage =
       (ushort)((2 - (uint)*(ushort *)&button->xshift & 3) << 7) | (short)(uStack30 & 0x100) >> 4 |
       (ushort)(((uint)uStack32 & 0x3ff) >> 6) | (ushort)(((uint)uStack30 & 0x200) << 2);
  *(_ButtonTexture **)&button->vramBlock->udata = button;
  local_1c = button->textureW;
  sStack26 = button->textureH;
                    /* WARNING: Subroutine does not return */
  LoadImage(&uStack32,plVar3 + 3);
}
