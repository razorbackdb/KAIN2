  ...
  if (((aadMem->dynamicSfxLoadInfo).flags & 1U) == 0) {
    ...
    if ((aadMem->numLoadReqsQueued == 0) || ((aadMem->sramDefragInfo).status != 0)) {
      ...
      if (pAVar8->type == 0) {
        ...
        (*aadMem->nonBlockLoadProc)
                  (string,aadLoadDynamicSfx,string,0,&(pAVar3->dynamicSfxLoadInfo).snfFile,0x2f);
