...
// void /*$ra*/ G2Quat_Slerp_VM(long ratio /*$s2*/, struct _G2Quat_Type *quatA /*$s3*/, struct _G2Quat_Type *quatB /*$s5*/, struct _G2Quat_Type *quatOut /*$s6*/, int spin /*stack 16*/)
 // line 93, offset 0x80095d38
	/* begin block 1 */
		// Start line: 94
		// Start offset: 0x80095D38
		// Variables:
	// 		long beta; // $s1
	// 		short theta; // $s0
	// 		short cos_t; // $a0
...
/* File: C:\kain2\game\PSX\PSX_G2\QUATVM.C */
...

{
  ...
  iVar5 = (uVar2 & 0xffff) + ((int)quatA->w * (int)quatB->w >> 0xc);
  ...
  if (0 < 0x1000 - (int)sVar7) {
    ...
    iVar3 = rsin((int)(short)iVar5);
    a = ratio * ((iVar5 + spin * 0x1000) * 0x10000 >> 0x10) >> 0xc;
    iVar5 = rsin((int)(short)iVar5 - a);
    iVar5 = (iVar5 << 0xc) / (int)(short)iVar3;
    a = rsin(a);
    ratio = (a << 0xc) / (int)(short)iVar3;
  }
  if (bVar1) {
    ratio = -ratio;
  }
  ...
  a = getCopReg(2,0xb);
  lVar4 = MATH3D_FastSqrt(iVar6 * iVar6 + iVar5 * iVar5 + iVar3 * iVar3 + a * a + 0x800 >> 0xc);
