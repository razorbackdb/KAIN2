...

{
  ...
  if ((uint)def->denomFlag != denomFlag) {
    if (def->fracCurr != 0) {
      ...
      if (denomFlag == 0) {
        transitionMusicEndCallback((long)spline,(int)def,0);
        return;
      }
      def->fracCurr = (uint)(def->fracCurr * iVar2) >> 0xf;
    }
...
/* WARNING: Removing unreachable block (ram,0x80041568) */
...

{
  ...
  bVar1 = iVar12 < iVar11;
  sVar10 = (short)iVar11;
  local_40.currkey = sVar10;
  if (bVar1) {
    local_40.currkey = (short)iVar12;
  }
  local_34 = (uint)bVar1;
  local_40.fracCurr = 0;
  local_40.denomFlag = 0;
  if (0 < spline->key[local_40.currkey].count) {
    local_30 = def;
                    /* WARNING: Subroutine does not return */
...

{
  ...
  if (def->currkey < 1) {
    ...
    def->currkey = uVar3;
    if (bVar2) goto LAB_80041b10;
LAB_80041ae4:
    sVar1 = (&spline->key->count)[(int)(short)uVar3 * 5];
  }
  else {
    def->currkey = uVar3;
    if (!bVar2) goto LAB_80041ae4;
LAB_80041b10:
...

{
  ...
  if (spline != (Spline *)0x0) {
    ...
    point = (_SVector *)(int)def->currkey;
    def_00 = (SplineDef *)(int)spline->numkeys;
    bVar2 = spline->type != '\x01';
    if ((int)point < (int)def_00) {
      uVar5 = 1;
      if (!bVar2) {
        uVar5 = SplineGetData(spline,(SplineDef *)point,def_00);
        return uVar5;
      }
      if (0 < spline->key[(int)point].count) {
        point = SplineGetNearestPoint(spline,point,def_00);
        return (ulong)point;
      }
      iVar4 = 1;
      SplineSetDefDenom(spline,def,0);
      ...
      do {
        ...
        if ((int)spline->numkeys + -1 < (int)def->currkey) {
          ...
          if ((int)def->currkey <= (int)spline->numkeys + -1) goto LAB_80041c88;
        }
        else {
LAB_80041c88:
          if (bVar2) {
