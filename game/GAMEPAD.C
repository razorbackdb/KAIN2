...

{
  ...
  if (gameTrackerX.setDemoMode != '\0')
  {
    ...
    __demoBufferStart = (u_short *)LOAD_ReadFileFromCD(acStack88, '\x11');
...
void PSXPAD_TranslateData(long *data, u_short padData, u_short lastData)

{
  ...
  do
  {
    uVar1 = *(u_short *)puVar5;
    uVar3 = (u_int)((u_short *)puVar5)[1];
    if ((padData & uVar1) == 0)
    {
      *data = *data | uVar3;
      if ((lastData & uVar1) != 0)
    ...
    iVar6 = iVar6 + 1;
    puVar5 = (u_char *)((u_short *)puVar5 + 2);
  } while (iVar6 < 0x10);
  return;
}

u_short GAMEPAD_RemapAnalogueButtons(u_short in)

{
  u_int uVar1;

  uVar1 = ~(u_int)in;
  return ~((u_short)uVar1 & 0x61f9 |
           (u_short)((uVar1 & 0x800) << 1) | (u_short)((uVar1 & 0x400) << 5) |
           (u_short)((uVar1 & 0x200) << 1) | (u_short)((uVar1 & 0x8000) >> 6) |
           (u_short)((uVar1 & 0x1000) >> 1));
}
